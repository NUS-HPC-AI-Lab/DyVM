...
def generate_random_binary_mask(shape, ratio, device, dtype):
    """
    Generate a random binary mask with a specified ratio of ones.
    For ablation studies only.
    """
    # Calculate the number of ones based on the ratio
    num_elements = torch.prod(torch.tensor(shape)).item()
    num_ones = int(num_elements * ratio)
    num_zeros = num_elements - num_ones
    
    # Create a tensor with the specified number of 1s and 0s
    tensor = torch.cat((torch.ones(num_ones), torch.zeros(num_zeros)))
    
    # Shuffle the tensor to randomize the positions of 1s and 0s
    tensor = tensor[torch.randperm(num_elements)]
    
    # Reshape to the desired shape
    return tensor.view(shape).to(device=device, dtype=dtype)