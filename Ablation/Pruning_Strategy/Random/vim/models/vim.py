...
from utils import combine_cls_and_other, extract_cls_and_other, rearrange_pos, batch_index_select, generate_random_binary_mask
...
class VisionMambaDiffPruning(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
            ...
            for i, layer in enumerate(self.layers):
                ...
                if i in self.pruning_loc:
                    if self.training:
                        ...
                        with torch.no_grad():
                            hard_keep_decision = generate_random_binary_mask(policy.size(), self.token_ratio[predictor_count], device=policy.device, dtype=policy.dtype)
                        ...
                        # pred_score = self.score_predictors[predictor_count](other_t + other_r, policy)
                        # score = pred_score[:, :, 0]
                        score = torch.rand(policy.size(), device=hidden_states.device, dtype=hidden_states.dtype).squeeze(-1)
        ...
        if self.training:
            ...
            return cls_t, other_t, policy.detach(), current_pos
        else:
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        if self.training:
            cls_t, other_t, policy, current_pos = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
            return self.head(cls_t), other_t, policy, current_pos
        else:
