...
from utils import combine_cls_and_other, extract_cls_and_other, rearrange_pos, batch_index_select, generate_even_spread_mask
            ...
            for i, layer in enumerate(self.layers):
                ...
                if i in self.pruning_loc:
                    if self.training:
                        ...
                        with torch.no_grad():
                            hard_keep_decision = generate_even_spread_mask(policy.size(), self.token_ratio[predictor_count]).to(policy.device, policy.dtype)
                        ...
                        # pred_score = self.score_predictors[predictor_count](other_t + other_r, policy)
                        # score = pred_score[:, :, 0]
                        score = generate_even_spread_mask(policy.size(), self.token_ratio[0]).squeeze(-1).to(policy.device, policy.dtype)
