...
def rearrange_pos(policy, other_tokens, other_residual, current_pos):
    ...
    zero_indices = (policy == 0).nonzero(as_tuple=False)
    one_indices = (policy == 1).nonzero(as_tuple=False)
    new_order = torch.empty_like(policy, dtype=torch.long)
    for i in range(B):
        new_order[i] = torch.cat(( one_indices[one_indices[:, 0] == i][:, 1], 
                                    zero_indices[zero_indices[:, 0] == i][:, 1]), dim=0)

    rearranged_policy = policy[torch.arange(B).unsqueeze(-1), new_order]
    rearranged_other_tokens = other_tokens[torch.arange(B).unsqueeze(-1), new_order]
    rearranged_other_residual = other_residual[torch.arange(B).unsqueeze(-1), new_order]
    rearranged_pos = current_pos[torch.arange(B).unsqueeze(-1), new_order]
...
def generate_even_spread_mask(input_size, ratio):
    # Separate batch size from the input shape
    batch_size = input_size[0]
    spatial_dims = input_size[1:]
    
    # Calculate the total number of elements (excluding batch dimension) and the target number of ones
    total_elements = torch.tensor(spatial_dims).prod().item()
    num_ones = int(total_elements * ratio)

    # Create a flattened tensor with the desired number of ones and zeros in a deterministic pattern
    mask = torch.zeros(total_elements)
    indices = torch.linspace(0, total_elements - 1, num_ones).long()
    mask[indices] = 1
    
    # Reshape to the spatial dimensions
    mask = mask.view(spatial_dims)
    
    # Repeat the mask across the batch dimension
    mask = mask.unsqueeze(0).repeat(batch_size, *[1] * len(spatial_dims))
