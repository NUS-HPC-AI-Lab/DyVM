...
class VisionMambaDiffPruning(nn.Module):
    def __init__(self, 
        ...
        self.score_predictors = nn.ModuleList(
            [
                PredictorLG(d_state) for _ in range(len(pruning_loc))
            ]
        )
            ...
            for i, layer in enumerate(self.layers):
                ...
                if i in self.pruning_loc:
                    ...
                    if self.training:
                        ...
                        _, mamba_B = extract_cls_and_other(mamba_B, cls_token_position)
                        pred_score = self.score_predictors[predictor_count](mamba_B, policy)
                        ...
                        mamba_B = torch.cat([mamba_B[:, :cls_token_position, :], mamba_B[:, cls_token_position+1:, :]], dim=1)
                        pred_score = self.score_predictors[predictor_count](mamba_B, policy)
                        score = pred_score[:, :, 0]
