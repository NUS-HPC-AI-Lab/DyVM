...
class DistillDiffPruningLoss_dynamic(torch.nn.Module):
    ...
    def forward(self, inputs, outputs, labels):
        ...
        for i, score in enumerate(pred_decisions):
            ...
            ratio_loss += ((pos_ratio - target_ratio[i]) ** 2).mean()
        ...
        loss = self.clf_weight * cls_loss + self.ratio_weight * ratio_loss / len(self.pruning_loc) + self.distill_weight * (KL_loss + token_distill_loss)  

        return loss, [self.clf_weight * cls_loss, self.ratio_weight * ratio_loss / len(self.pruning_loc), self.distill_weight * KL_loss, self.distill_weight * token_distill_loss]
