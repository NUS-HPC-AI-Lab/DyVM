...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, mask=None):
        ...
        if mask is not None:
            ...
            mask_b = mask.flip([-1])
        out, forward_delta, forward_B, forward_C = mamba_inner_with_mask(
            xz,
            self.conv1d.weight,
            self.conv1d.bias,
            self.x_proj.weight,
        ...
        out_b, backward_delta, backward_B, backward_C = mamba_inner_with_mask(
            xz.flip([-1]),
        ...
        mamba_B = rearrange((forward_B + backward_B.flip([-1])) / 2, "b 1 dstate l -> b l dstate")
        mamba_C = rearrange((forward_C + backward_C.flip([-1])) / 2, "b 1 dstate l -> b l dstate")
        mamba_delta = rearrange((forward_delta + backward_delta.flip([-1])) / 2, "b d l -> b l d")

        if self.init_layer_scale is not None:
                out = out * self.gamma    
        
        return out, (mamba_delta, mamba_B, mamba_C)
