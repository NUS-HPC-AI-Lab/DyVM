...
class Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states, mamba_mid_results = self.mixer(hidden_states, inference_params=inference_params, mask=mask)
        return hidden_states, residual, mamba_mid_results
...
class VisionMambaDiffPruning(nn.Module):
    def __init__(self, 
                 img_size=224, 
                 patch_size=16, 
                 stride=16,
        ...
        self.score_predictors = nn.ModuleList(
            [
                PredictorLG(2 * self.embed_dim) for _ in range(len(pruning_loc))
            ]
        )
        ...
        if not self.if_bidirectional:
            for i, layer in enumerate(self.layers):
                ...
                if i in self.pruning_loc:
                    mamba_delta, mamba_B, mamba_C = mamba_mid_results
                    if self.training:
                        ...
                        _, mamba_delta = extract_cls_and_other(mamba_delta, cls_token_position)
                        pred_score = self.score_predictors[predictor_count](mamba_delta, policy)
                        ...
                        mamba_delta = torch.cat([mamba_delta[:, :cls_token_position, :], mamba_delta[:, cls_token_position+1:, :]], dim=1)
                        pred_score = self.score_predictors[predictor_count](mamba_delta, policy)
                        score = pred_score[:, :, 0]
                ...
                hidden_states, residual, mamba_mid_results = layer(
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        if self.training:
            cls_t, other_t, policy, pred_decisions, current_pos = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
...
@register_model
def VimTinyDiffPruning(pretrained=False, **kwargs):
    model = VisionMambaDiffPruning(
...
@register_model
def VimSmallDiffPruning(pretrained=False, **kwargs):
    model = VisionMambaDiffPruning(
...
@register_model
def VimBaseDiffPruning(pretrained=False, **kwargs):
    model = VisionMambaDiffPruning(
