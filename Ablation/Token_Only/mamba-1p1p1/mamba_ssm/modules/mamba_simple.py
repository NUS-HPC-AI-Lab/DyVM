...
class DynamicMamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, mask=None, cls_token_position=None):
        ...
        # [B, 1, D]
        cls_tokens = head_input[torch.arange(batch), cls_token_position].unsqueeze(1)
        ...
        if self.training:
            ...
            forward_indices = (forward_mask.squeeze(-1).squeeze(-1) == 1).nonzero(as_tuple=True)[0]
        ...
        if self.training:
            ...
            backward_indices = (backward_mask.squeeze(-1).squeeze(-1) == 1).nonzero(as_tuple=True)[0]
