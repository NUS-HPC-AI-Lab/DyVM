...
from engine import train_one_epoch_token_only as train_one_epoch
from engine import evaluate_token_only as evaluate
from losses import DyVMLoss_Token_Only as DyVMLoss
...
def main(args):
    ...
    if args.model.startswith('Vim'):
        ...
        if not args.eval:
            ...
            utils.load_state_dict(teacher_model, checkpoint['model'], ignore_missing='score_predictors')
    ...
    print('# FLOPs test')
    flops = get_flops(model, img_size=args.input_size)
    print(f"FLOPs: {flops / 1e9:.2f}G")

    flops = get_flops(teacher_model, img_size=args.input_size)
    ...
    if args.eval:
        ...
        throughput(model, device, 256, img_size=args.input_size)
