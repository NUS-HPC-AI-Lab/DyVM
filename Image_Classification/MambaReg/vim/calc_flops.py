...
@torch.no_grad()
def get_throughput(model, device, batch_size, img_size=224):
    model.eval()
    ...
    return batch_size * 30 / (tic2 - tic1)

def get_flops(model, img_size=224, block_keep_ratio=1.0, show_detail=False):
    conv_flops = []
    ...
    def dynamic_mamba_hook(self, input, output, path=None):
        if show_detail:
            print(f'{path} is called')
        ...
        mamba_flops.append(flops)
        if show_detail:
            print(f'Dynamic Mamba flops: {flops}')
    ...
    def register_module_hook(net, hook_handle, prefix='', path=''):
        for name, module in net.named_children():
            ...
            if isinstance(module, Mamba):
                ...
                registerd = True
            if isinstance(module, DynamicMamba):
                hook_handle.append(module.register_forward_hook(lambda *args, path=path + '/' + name + ':' + module.__class__.__name__: dynamic_mamba_hook(*args, path=path)))
                registerd = True
