...
def combine_cls_and_other(cls_tokens, other_tokens, cls_token_pos, num_cls_token=1):
    if num_cls_token == 1:
        ...
        B, _, D = other_tokens.shape
        assert cls_token_pos.shape == (B, num_cls_token)
        original_length = other_tokens.shape[1] + num_cls_token
    
        # Initialize the merged tensor
        merged_tensor = torch.empty(B, original_length, D, device=other_tokens.device)

        # Create a mask for non-CLS tokens
        mask = torch.ones(B, original_length, dtype=torch.bool, device=other_tokens.  device)
        batch_indices = torch.arange(B, device=other_tokens.device).unsqueeze(1)
        mask[batch_indices, cls_token_pos] = False  # Mark CLS positions as False

        # Place non-CLS tokens
        merged_tensor[mask] = other_tokens.reshape(-1, D)

        # Place CLS tokens
        merged_tensor[~mask] = cls_tokens.reshape(-1, D)

        return merged_tensor


def extract_cls_and_other(tokens, cls_token_pos, num_cls_token=1):
    if num_cls_token == 1:
        ...
        # Create a mask for non-CLS tokens
        mask = torch.ones(B, L, dtype=torch.bool, device=tokens.device)
        batch_indices = torch.arange(B, device=tokens.device).unsqueeze(1)

        mask[batch_indices, cls_token_pos] = False  # Mark CLS positions as False

        # Gather non-CLS tokens
        other_tokens = tokens[mask].view(B, L - num_cls_token, D)

        # Gather CLS tokens
        cls_tokens = tokens[~mask].view(B, num_cls_token, D)

        return cls_tokens, other_tokens

def rearrange_pos(policy, other_tokens, other_residual, current_pos, num_cls_token=1):
    B, _, _ = other_tokens.shape
    ...
    if num_cls_token == 1:
        ...
        interval = (num_keep // (num_cls_token + 1)).unsqueeze(-1)
        cls_token_pos = torch.cat([(interval + 1) * (i + 1) - 1 for i in range(num_cls_token)], dim=-1)
