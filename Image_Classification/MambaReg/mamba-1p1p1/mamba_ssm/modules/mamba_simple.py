...
class DynamicMamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, mask=None, cls_token_position=None):
        ...
        # [B, 1, D]
        if cls_token_position.shape == torch.Size([batch]):
            cls_tokens = head_input[torch.arange(batch), cls_token_position].unsqueeze(1)
        else:
            batch_indices = torch.arange(batch, device=cls_token_position.device).unsqueeze(1)
            cls_tokens = head_input[batch_indices, cls_token_position].mean(dim=1, keepdim=True)
