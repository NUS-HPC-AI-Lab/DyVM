...
class DyVM(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.training:
            ...
            return cls_t, other_t, token_policy.detach(), pred_decisions, current_pos, block_policy
        else:
            cls_t = hidden_states[:, cls_token_position, :]
            block_policy = torch.stack(block_policies, dim=0)
            return cls_t, block_policy
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        if self.training:
            cls_t, other_t, token_policy, pred_decisions, current_pos, block_policy = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
            return self.head(cls_t), other_t, token_policy, pred_decisions, current_pos, block_policy
        else:
            cls_t, block_policy = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
